"""Bybit long/short ratio provider with 429-aware retry.
...
def _log_retry(retry_state) -> None:
    ...
    logger.warning(
        "Bybit LS-ratio HTTP retry attempt=%d waiting=%.1fs after %r",
...
class BybitLSRatioProvider(_BaseRESTProvider):
    """Bybit v5 long/short account-ratio REST provider (1d period).
    ...
    def _fetch_range(self, symbol: str, start: str, end: str) -> pd.DataFrame:
        """Fetch all long/short account-ratio records in ``[start, end]``.

        Bybit's ``/v5/market/account-ratio`` endpoint caps at 200 records per
        request (~200 days for the ``1d`` period). Multi-year backfills are
